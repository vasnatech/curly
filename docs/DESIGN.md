...
4. **M4 — GUI v1** 🚧 **started, in progress**: FR-17..24 against the by-then-stable core. New `curly-gui` lib crate (not a separate binary — linked into the `curly` binary per §3, gated behind a `gui` Cargo feature, default-on; `cargo build --no-default-features` gives the lean CLI-only build). `eframe`/`egui` 0.36.1 resolved (note: this version's `App` trait method is `ui(&mut self, ui: &mut egui::Ui, frame: &mut Frame)`, not the `update(&mut self, ctx: &egui::Context, ...)` shape from older egui docs/examples still circulating — `CentralPanel::show` takes `&mut Ui` too, nesting rather than taking `&Context`; worth knowing before consulting older egui examples). `curly` launches the GUI when invoked with zero arguments at all (checked via `std::env::args().len() <= 1`, before clap even runs) or via explicit `curly gui`; any other flag/subcommand combination behaves exactly as before.
   ...
   - **What's not built yet**: multiple simultaneously-open projects, defining/editing extraction rules from the GUI (only *running* a saved request's existing rules is supported — authoring them is still `curly collections add-request --extract-*`-only), FR-19 (multiple request tabs), FR-20 (save/rename/duplicate/move from the GUI), FR-22 (syntax highlighting, search, image preview — response body is plain monospace text), FR-23 (Ctrl+S; Ctrl+Enter-to-send is done), FR-24 (light/dark theme — currently whatever `eframe::NativeOptions::default()` gives). **FR-21 itself is now fully done** in the GUI: switch/create/edit/delete environments, ad-hoc `--var` overrides, and extraction-into-session with a way to see the result.
   - 18 new tests (204 across the workspace; the icon slice added none — it's asset embedding and viewport config, not logic to unit-test) — the extraction slice's cover `load_saved_request` carrying rules over (and clearing a stale `session_notice`), `send` capturing the right rules/session-scope before the request goes out (using `egui::Context::default()`, since that part is synchronous and needs no real event loop), `apply_pending_extraction` writing into the session on 2xx, skipping silently on non-2xx, being a no-op with nothing pending, and reporting an extraction failure or a missing-project error; the session-panel slice's cover the display refreshing on a successful extraction (and *not* refreshing on a failed one), on environment selection/deselection (falling back to the `global` session), on opening a new project, `clear_session` removing the file and refreshing the display (and being a safe no-op with nothing open), and `session_scope()`'s own default. Verified live by the user on their own desktop, ten times now — most recently in two back-to-back rounds: extraction itself (a real `TOKEN` landed in the `local` session after a real `Auth/login`, confirmed via `curly session show --env local`), then the new Session panel after the first round's "I don't see it in the GUI" report turned out to be exactly the missing-visibility gap described above.
