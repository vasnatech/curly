...
4. **M4 — GUI v1** 🚧 **started, first slice only**: FR-17..24 against the by-then-stable core. New `curly-gui` lib crate (not a separate binary — linked into the `curly` binary per §3, gated behind a `gui` Cargo feature, default-on; `cargo build --no-default-features` gives the lean CLI-only build). `eframe`/`egui` 0.36.1 resolved (note: this version's `App` trait method is `ui(&mut self, ui: &mut egui::Ui, frame: &mut Frame)`, not the `update(&mut self, ctx: &egui::Context, ...)` shape from older egui docs/examples still circulating — `CentralPanel::show` takes `&mut Ui` too, nesting rather than taking `&Context`; worth knowing before consulting older egui examples). `curly` launches the GUI when invoked with zero arguments at all (checked via `std::env::args().len() <= 1`, before clap even runs) or via explicit `curly gui`; any other flag/subcommand combination behaves exactly as before.
   ...
   - **What's not built yet**: FR-18 (collections/history/environments sidebar), FR-19 (multiple request tabs), FR-20 (save/rename/duplicate/move), FR-21 (environment editor), FR-22 (syntax highlighting, search, image preview — response body is plain monospace text for now), FR-23 (Ctrl+S; Ctrl+Enter-to-send is done), FR-24 (light/dark theme — currently whatever `eframe::NativeOptions::default()` gives, no explicit theming yet). In short: the GUI can send a request and show a response, and nothing else — storage/collections integration is the natural next slice, the same way the CLI's own M1 was one-shot-only before M2 added storage.
   - 9 new tests (139 across the workspace) — `build_request`'s pure logic (method parsing/case-insensitivity, header enabled/blank-name filtering, body inclusion). The render loop itself isn't unit tested (egui's immediate-mode UI isn't practical to unit test — DESIGN.md §8 already anticipated this: "GUI: manual testing only for v1"). Verified live: the compiled binary launches without error under this environment's actual Wayland/GNOME Shell session, the zero-args/`curly gui`/explicit-flag-without-URL/`--no-default-features` dispatch branches all precisely via process/exit-code checks, and — since no screenshot tool was available in the sandbox this was built in to see the window directly — **the user ran it themselves on their own desktop**: confirmed the window renders correctly and a live GET against httpbin.org round-tripped end to end with the body pretty-printed as expected. The send/response loop is human-confirmed working, not just inferred from clean process exit; exact pixel layout remains code-derived only.
