...
- Extraction rules writing into a session from the GUI (the last piece of FR-21) — the GUI reads sessions the CLI's `curly run --extract-*` already wrote, but doesn't run extraction itself yet
