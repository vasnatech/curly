...
4. **M4 — GUI v1** 🚧 **started, in progress**: FR-17..24 against the by-then-stable core. New `curly-gui` lib crate (not a separate binary — linked into the `curly` binary per §3, gated behind a `gui` Cargo feature, default-on; `cargo build --no-default-features` gives the lean CLI-only build). `eframe`/`egui` 0.36.1 resolved (note: this version's `App` trait method is `ui(&mut self, ui: &mut egui::Ui, frame: &mut Frame)`, not the `update(&mut self, ctx: &egui::Context, ...)` shape from older egui docs/examples still circulating — `CentralPanel::show` takes `&mut Ui` too, nesting rather than taking `&Context`; worth knowing before consulting older egui examples). `curly` launches the GUI when invoked with zero arguments at all (checked via `std::env::args().len() <= 1`, before clap even runs) or via explicit `curly gui`; any other flag/subcommand combination behaves exactly as before.
   ...
   - **What's not built yet**: multiple simultaneously-open projects, editing/creating environments or `--var`-style overrides from the GUI (rest of FR-21), extraction rules writing into a session from the GUI (the GUI reads sessions the CLI already wrote, doesn't populate them itself), FR-19 (multiple request tabs), FR-20 (save/rename/duplicate/move from the GUI), FR-22 (syntax highlighting, search, image preview — response body is plain monospace text), FR-23 (Ctrl+S; Ctrl+Enter-to-send is done), FR-24 (light/dark theme — currently whatever `eframe::NativeOptions::default()` gives).
   - 34 new tests (169 across the workspace; the icon slice added none — it's asset embedding and viewport config, not logic to unit-test) — the project slice's are pure filesystem-backed logic (`open_project_at`/`set_active_project`/`refresh_project_lists`, via `tempfile`, deliberately kept out of `default_state` — see its doc comment — so unit tests never depend on whatever real `.curly` might exist above wherever `cargo test` happens to run from); the browsing slice's cover `load_saved_request`'s notice logic; the environment-switcher slice's cover `merged_variables`'s precedence (global-only, selected-overrides-global, selected-environment's session, global-session-fallback-when-unselected), `build_request` substituting correctly and erroring on an undefined variable, switching environments changing what a send resolves to, a new project resetting the selection, and the load-notice clearing once the selected environment defines what was missing. Verified live by the user on their own desktop, five times now — most recently this slice, end-to-end against the real `health-record-backend-spring` project: loading its actual `Auth/login` request (`{{BASE_URL}}` + a JSON body referencing `{{EMAIL}}`/`{{PASSWORD}}`), selecting its `local` environment, sending, and getting back a real JWT from the real local backend. An intermediate 400 turned out to be a typo in that environment's stored `EMAIL` test value (`curly@@test.local`, a stray double `@`) rather than a substitution bug — confirmed by fixing the stored variable (`curly env set local EMAIL=...`) and getting a successful login on retry with no GUI restart needed, since environments are read fresh from disk on every send.
