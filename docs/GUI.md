...
> Status: **early, several slices so far, FR-21 now complete** (M4, still in progress — see [DESIGN.md](DESIGN.md) §6/§9). The GUI has a toolbar (open a project, toggle light/dark, zoom the whole UI), supports multiple request tabs open at once, can open a project, browse its collections as a tree, click a saved request to load it into a tab (including its extraction rules), create/edit/delete environments and switch between them (plus ad-hoc `--var`-style overrides) to resolve `{{variable}}` tokens, build a request from scratch, save it into a collection (new or overwriting what's loaded) or delete it, send it, show the response with JSON syntax highlighting and in-body search, save the response to a file, and run a loaded request's extraction rules on success and write the results into a session (viewable read-only in the sidebar). It does **not** yet open more than one project at once, or preview an image response, and there's no rename/duplicate/move for an existing saved request yet — those are still FR-20 (partly)/22 (partly). If you need history today, use [the CLI](CLI.md) — GUI and CLI share the same underlying storage, so anything you build or extract with one is already usable by the other.
...
A single row across the top of the window holds every "global" action — not scoped to any particular project, request, or panel:

- **Open Project…** — see "Projects" below.
- **☀/🌙** — light/dark theme toggle; see "Theme" below.
- **Zoom: − 100% +  Reset** — scales the *entire* UI (fonts, spacing, every widget) up or down. **100% is the baseline this app is laid out for**, at a 1024×768 window — if everything reads small on a high-DPI/high-resolution display, this is the fix, not a font-size setting buried somewhere. Click `−`/`+` (each click is a ~10% step), or use **Ctrl/Cmd + minus/plus/equals** (`+` and `=` both work, since `+` needs Shift on most keyboards), or **Ctrl/Cmd + scroll wheel** anywhere in the window (egui's own built-in gesture, wired up to actually apply here — see DESIGN.md for why that's a separate step from the gesture itself), or **Ctrl/Cmd+0** to reset to exactly 100%. The percentage shown always reflects the real current value. Zoom isn't saved between launches yet — it resets to 100% every time you start curly.

### Request tabs (FR-19)

A row of tabs sits above the method/URL bar — each one is a fully independent request: its own method, URL, headers, body, response, search box, extraction rules, and ad-hoc `--var` overrides. Nothing about one tab affects another.

- **Click a tab** to switch to it; the toolbar/sidebar (project, environments, collections) stay the same across all tabs — only the request editor and response pane are per-tab.
- **✕** on a tab closes it immediately, no confirmation — closing the *last* remaining tab resets it to blank instead of leaving zero tabs open, the same way a browser won't let you close its only tab.
- **+** (or **Ctrl/Cmd+T**) opens a new blank tab and switches to it.
- Each tab's label shows the loaded request's name if it has one, otherwise `METHOD url`, otherwise "New Request" — long labels are truncated.
- Environments/overrides/extraction rules are **per-tab**, not global — load two different requests into two tabs and each keeps its own `--var` overrides and its own "will extract..." note independently.
- **Sending from a tab you then switch away from keeps running** — the request doesn't get cancelled, but its response only becomes visible once you switch back to that tab (there's no "response ready" badge on a background tab yet).
- Opening the Save dialog, then switching tabs, cancels the dialog rather than risk saving the *new* active tab's content under the old tab's pre-filled name/collection.
...
**Only one project is open at a time right now** — picking a new one replaces the current one. This is deliberate groundwork, not a missing feature that slipped through: the underlying state is already a list (so a future "open several projects, switch between them" doesn't need a data-model change, just a UI for it), but only single-project interaction is wired up so far. This is a different, independent axis from request tabs above — one project, multiple request tabs into it, today.
...
- **New** — resets the *current tab* to a fully blank request (method back to GET, headers/body/extraction rules/overrides/search all cleared) without touching the active project or environment selection. Equivalent to opening a new tab (Ctrl/Cmd+T) except it reuses the current tab instead of adding one — use whichever fits; both leave you with a blank slate so a later "Save" can't overwrite the wrong thing.
...
- **Response pane** — status (color-coded: green 2xx, yellow 3xx, red 4xx/5xx), elapsed time, body size, a collapsible list of response headers, and the body — pretty-printed automatically if it's JSON, shown as-is otherwise. See "Reading a response" below for the search/highlighting/save-to-file that comes with it.
...
### Reading a response (FR-22, partial)

- **JSON syntax highlighting** — a response body that parses as JSON gets its strings, numbers, `true`/`false`/`null`, and punctuation colored differently (colors chosen to work in both light and dark theme). A non-JSON body (HTML, XML, plain text, ...) still displays fine, just without the coloring.
- **Search** — the "Search:" box above the body highlights every case-insensitive match with a yellow background and shows a "N match(es)" count next to it. Works on any response body, JSON or not; there's no "find next"/scroll-to-match yet, just highlight-all.
- **Save Response…** — opens a native "Save As" dialog (async, same responsiveness fix as "Open Project…" below) and writes the raw response body to wherever you pick. Defaults the suggested filename to `response.json` if the body parsed as JSON, `response.txt` otherwise — you can change either the name or the extension in the dialog. A green notice confirms the path once it's written; a cancelled dialog shows nothing.

**Not built**: image preview. `ResponseSummary` (the shared type `curly-core`'s HTTP layer hands both the CLI and the GUI) stores the body as a `String`, decoded as text — there's no raw-bytes path for a response, so an image response can't be represented correctly today. Supporting preview means changing that shared type, which would touch the CLI too; deliberately out of scope for what was meant to be a GUI-only slice.
...
- A "response ready" indicator on a background tab whose send just completed while you were looking at a different tab (see "Request tabs" above)
...
- Image preview for a response, and "find next"/scroll-to-match for search (rest of FR-22) — syntax highlighting, highlight-all search, and save-to-file are done; see "Reading a response" above
