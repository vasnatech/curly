...
4. **M4 — GUI v1** 🚧 **started, first slice only**: FR-17..24 against the by-then-stable core. New `curly-gui` lib crate (not a separate binary — linked into the `curly` binary per §3, gated behind a `gui` Cargo feature, default-on; `cargo build --no-default-features` gives the lean CLI-only build). `eframe`/`egui` 0.36.1 resolved (note: this version's `App` trait method is `ui(&mut self, ui: &mut egui::Ui, frame: &mut Frame)`, not the `update(&mut self, ctx: &egui::Context, ...)` shape from older egui docs/examples still circulating — `CentralPanel::show` takes `&mut Ui` too, nesting rather than taking `&Context`; worth knowing before consulting older egui examples). `curly` launches the GUI when invoked with zero arguments at all (checked via `std::env::args().len() <= 1`, before clap even runs) or via explicit `curly gui`; any other flag/subcommand combination behaves exactly as before.
   ...
   - 9 new tests (139 across the workspace) — `build_request`'s pure logic (method parsing/case-insensitivity, header enabled/blank-name filtering, body inclusion). The render loop itself isn't unit tested (egui's immediate-mode UI isn't practical to unit test — DESIGN.md §8 already anticipated this: "GUI: manual testing only for v1"). Verified live: the compiled binary launches and runs without error under this environment's actual Wayland/GNOME Shell session (confirmed via process liveness + empty stderr across two runs, since no screenshot tool was available in this sandbox to capture Wayland surfaces non-interactively — a real gap in *this session's* verification, not a defect in the app); the zero-args/`curly gui`/explicit-flag-without-URL/`--no-default-features` dispatch branches were all verified precisely via process/exit-code checks.
